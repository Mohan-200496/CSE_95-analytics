...
async def create_users_sql_direct():
    try:
        ...
        async with AsyncSession(async_engine) as session:
            ...
            # Get the employer's database ID
            result = await session.execute(text("SELECT id FROM users WHERE email = 'employer@test.com'"))
            employer_db_id = result.fetchone()[0]
            
            # Create company for employer
            await session.execute(text("""
                INSERT INTO companies (
                    user_id, company_name, industry, company_size, location, description,
...
if __name__ == "__main__":
    asyncio.run(create_users_sql_direct())